        ...
        for (int nc = 0; nc < NC; nc++) { \
            for (int i = 0; i < IMG_SIZE_IN; i += STRIDE) { \
                for (int j = 0; j < IMG_SIZE_IN; j += STRIDE) { \
                    ...
                    double max_val = data_in[index_start]; \
...
void MAXPOOL_1_HARDWARE (
	double data_in[NC_MAXPOOL1*IMG_SIZE_MAXPOOL1_in*IMG_SIZE_MAXPOOL1_in],
    double data_out[NC_MAXPOOL1*IMG_SIZE_MAXPOOL1_out*IMG_SIZE_MAXPOOL1_out]
);

void MAXPOOL_2_HARDWARE (
	double data_in[NC_MAXPOOL2*IMG_SIZE_MAXPOOL2_in*IMG_SIZE_MAXPOOL2_in],
    double data_out[NC_MAXPOOL2*IMG_SIZE_MAXPOOL2_out*IMG_SIZE_MAXPOOL2_out]
);

void MAXPOOL_3_HARDWARE (
	double data_in[NC_MAXPOOL3*IMG_SIZE_MAXPOOL3_in*IMG_SIZE_MAXPOOL3_in],
